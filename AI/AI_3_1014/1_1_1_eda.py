...
# 8. color intensity가 10% 이상인 샘플의 비율 구하기
# 필터 진행
filtered_df = df['color_intensity'] >= 10
# 원본에서 시리즈가 True인 것들만 뽑아서 샘풀을 가져오기
...
# 10. Proline의 분포에서 가장 높은 피크를 가지는 클래스 번호를 구하기
# 히스토그램
...
# plt.title('Flavonoid Distribution')
# plt.show()
...
# 세번째 구간의 최솟값 + IQR * 1.5
# 를 벗어나는 데이터를 이상치로 판단
def detect_outliners_iqr(data, column):
    # data = df
    # column = 'alcohol'
...
outliers_alcohol = detect_outliners_iqr(df_missing, 'alcohol')
...
outliers_alcohol = detect_outliners_iqr(df_no_outliers, 'alcohol')
