...
# 사거리 내에 바위가 있나? line 203 <- not_wall(r, c, wall, grid)
def not_wall(r, c, wall, grid):
    WALL_DIRS = [(0,1), (1,0), (0,-1), (-1,0), (0,2), (2,0), (0,-2), (-2,0)]
    rows, cols = len(grid), len(grid[0])
...
# 경로 탐색 알고리즘
def bfs(grid, start, target, wall, water, tree):
    rows, cols = len(grid), len(grid[0])
    queue = deque([(start, [])])
    visited = {start}
    ...
    while queue:
        ...
        for d, (dr, dc) in enumerate(DIRS):            # 발사 for문
            nr, nc = r + dr, c + dc                         # 다음 위치 삽입
            if (nr, nc) == target:   # 사거리 내에 있고 사이에 벽이 없다면
                return actions + [FIRE_CMDS[d]]
            elif (nr, nc) == tree:
                return actions + [MEGA_FIRE_CMDS[d]]


        for d, (dr, dc) in enumerate(DIRS):      # 이동 for문
            nr, nc = r + dr, c + dc              # 다음 위치 삽입

            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] != wall and grid[nr][nc] != water and grid[nr][nc] != tree and (nr, nc) not in visited:
                visited.add((nr, nc))
...
# FIRE_DIRS = [(0,3), (3,0), (0,-3), (-3,0)]              # 사정거리  
...
TARGET = 'X'
WALL = 'R'
WATER = 'W'         # 물을 만났을시에 피하도록 변수 설정 후 각 함수의 매개변수에도 추가
TREE = 'T'       # 나무를 만났을시에 피하도록 각 함수의 매개변수에도 추가
...
# 출발지점, 목표지점의 위치 확인
start, target = find_positions(map_data, START, TARGET)
...
# 최초 경로 탐색
actions = bfs(map_data, start, target, WALL, WATER, TREE)
...
while game_data is not None:
    ...
    if not actions:
        start, target = find_positions(map_data, START, TARGET)
        actions = bfs(map_data, start, target, WALL, WATER, TREE) if start and target else []
